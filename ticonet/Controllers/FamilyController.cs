...
namespace ticonet
{
    public class FamilyController : Controller
    {
        ...
        public ActionResult create(tblFamily c)
        {
            ...
            {
                ViewBag.message = DictExpressionBuilderSystem.Translate("general.IdIsExist");
            ...
            {   ViewBag.message = DictExpressionBuilderSystem.Translate("general.emailIsExist");
              //  ViewBag.message = "האימייל של מבצע ההרשמה כבר קיימת במערכת- לא ניתן להבצע רישום ";
                return View(c);
            }
            if (c.iAgree == false)
            {   
                ViewBag.message =DictExpressionBuilderSystem.Translate("general.youMustReadAndApruve");
        ...
        public ActionResult update(familyViewModel c)
        {
            ...
            if (c != null)
            {
                ...
                using (tblFamilyLogic family = new tblFamilyLogic())
                {
                    ...
                    {
                        if (!Regex.IsMatch(c.EditableTblFamily.parent1Email, @"\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"))
                        {
                            ViewBag.message = DictExpressionBuilderSystem.Translate("general.emailIsNotProper");
                    ...
                    {
                        if (!Regex.IsMatch(c.EditableTblFamily.parent2Email, @"\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"))
                        {
                            ViewBag.message = DictExpressionBuilderSystem.Translate("general.emailIsNotProper");
                            return RedirectToAction("index", "Family");
                        }
                    ...
                    v.oneParentOnly = c.EditableTblFamily.oneParentOnly;
